...
const dbQuery = require('./catagories_db');
...
const getData = async (catagory) => {
  const product = await dbQuery.displayProduct(catagory);
...
function insertData(totalPage, paging, product_set, getDataResult) {
  ...
  for (let i = start; i < end; i++) {
    ...
    for (let k = 0; k < main_image.length; k++) {
      if (main_image[k].product_id == product_id) {
        inputMainImage.push(main_image[k].main_image);
      }
    }
...
//         var data = toObject(product_keys[i],product_values[i]);
...
//         for (var j=0; j<Data.data[i].variant.length; j++){
//             delete Data.data[i].variant[j].product_id;
//         }
//         for (var j=0; j<Data.data[i].color.length; j++){
//             delete Data.data[i].color[j].product_id;
//         }
//     }
